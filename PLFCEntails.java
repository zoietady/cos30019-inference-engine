...
public class PLFCEntails {
    ...
    public boolean isEntailed(KnowledgeBase kb, Sentence q) {
        ...
        Map<Sentence, Set<Sentence>> pToClausesWithPInPremise = initializeIndex(count, inferred);
        ...
        while (!agenda.isEmpty()) {
            ...
            if (inferred.get(p).equals(Boolean.FALSE)) {
                ...
                for (Sentence c : pToClausesWithPInPremise.get(p)) {
                    // decrement count[c]
                    decrement(count, c);
    ...
    private Map<Sentence, Set<Sentence>> initializeIndex(Map<Sentence, Integer> count,Map<Sentence, Boolean> inferred) {
    ...
    private Sentence getAgenda(ComplexSentence cs){
